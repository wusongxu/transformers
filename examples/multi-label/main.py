...
from transformers import (WEIGHTS_NAME, BertConfig,
                          ...
                          XLNetTokenizer)
from multi_label_model import BertForMultiLable
from metrics import AccuracyThresh, MultiLabelReport, F1Score
from transformers import AdamW, get_linear_schedule_with_warmup

from utils import output_modes, processors, compute_metrics, convert_examples_to_features
...
label_map = ['为何召回快递', '主播推荐', '优惠券无法领', '使用及教学视频', '使用周期', '保修卡咨询', '免息活动时间', '发送截图', '合作推广', '商品分类', '商品日期',
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.output_mode == "classification":
            ...
            # metric2 = MultiLabelReport(id2label)
            # metric2(logits=torch.from_numpy(preds), target=torch.from_numpy(out_label_ids))
            # metric2.value()


            metric3 = F1Score(search_thresh=True, average='macro')
            thresh, f1 = metric3(logits=torch.from_numpy(preds), target=torch.from_numpy(out_label_ids))
            results['f1'] = f1


            metric = AccuracyThresh(args.threshold)
            metric(logits=torch.from_numpy(preds), target=torch.from_numpy(out_label_ids))
            value = metric.value()
            results['acc'] = value
            metric.classification_report()


        # classification 和 regression 使用
...
def main():
    ...
    parser.add_argument("--logging_dir", type=str, default=50, help='')
    ...
    model = BertForMultiLable.from_pretrained(args.model_name_or_path, num_labels=len(label_list))
